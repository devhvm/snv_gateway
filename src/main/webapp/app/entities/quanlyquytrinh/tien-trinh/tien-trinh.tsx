...
import { getEntities } from './tien-trinh.reducer';
import { ITienTrinh } from 'app/shared/model/quanlyquytrinh/tien-trinh.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';

export interface ITienTrinhProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type ITienTrinhState = IPaginationBaseState;

export class TienTrinh extends React.Component<ITienTrinhProps, ITienTrinhState> {
  state: ITienTrinhState = {
  ...
  render() {
    const { tienTrinhList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="tien-trinh-heading">
          <Translate contentKey="gatewayApp.quanlyquytrinhTienTrinh.home.title">Tien Trinhs</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="gatewayApp.quanlyquytrinhTienTrinh.home.createLabel">Create new Tien Trinh</Translate>
          </Link>
        </h2>
        <div className="table-responsive">
          <Table responsive>
            ...
            <tbody>
              {tienTrinhList.map((tienTrinh, i) => (
                <tr key={`entity-${i}`}>
                  <td>
                    <Button tag={Link} to={`${match.url}/${tienTrinh.id}`} color="link" size="sm">
                      {tienTrinh.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${tienTrinh.id}`} color="info" size="sm">
                        <FontAwesomeIcon icon="eye" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.view">View</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${tienTrinh.id}/edit`} color="primary" size="sm">
                        <FontAwesomeIcon icon="pencil-alt" />{' '}
                        <span className="d-none d-md-inline">
                          <Translate contentKey="entity.action.edit">Edit</Translate>
                        </span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${tienTrinh.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ tienTrinh }: IRootState) => ({
  tienTrinhList: tienTrinh.entities,
  totalItems: tienTrinh.totalItems
});
